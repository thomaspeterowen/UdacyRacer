...
function setupClickHandlers() {
	document.addEventListener('click', function(event) {
		...
		if (target.matches('li.card.track')) {
...
// This async function controls the flow of the race, added logic and error handling
async function handleCreateRace() {
	...
	try{
		// Get racer and track data
		const racers = await getRacers();
		const tracks = await getTracks();
		const track = tracks[track_id - 1];
		...
		console.log("Problem with race setup ::", error.message)
		console.error(error)	
	}
}
	...
	return new Promise(resolve => {
		...
		const raceInfoInterval = setInterval( async () => {
			try{
				...
				console.log("Problem with race interval setup ::", error.message)
				console.error(error)	
			}
		}, 500);
	})
...
// fetch calls (with error handling!) to each of the following API endpoints 
